...
function Parser:statement(scope, currentLoop)
	...
	-- Goto Statement - only valid in Lua 5.4
	if(self.luaVersion == LuaVersion.Lua54 and consume(self, TokenKind.Keyword, "goto")) then
		local labelIdent = expect(self, TokenKind.Ident);
		return Ast.GotoStatement(labelIdent.value, scope);
	end

	-- Label Statement (::label::) - only valid in Lua 5.4
	if(self.luaVersion == LuaVersion.Lua54 and consume(self, TokenKind.Symbol, "::")) then
		local labelIdent = expect(self, TokenKind.Ident);
		expect(self, TokenKind.Symbol, "::");
		return Ast.LabelStatement(labelIdent.value, scope);
	end
	...
	if(consume(self, TokenKind.Keyword, "for")) then
		...
		local ids, attributes = self:nameList(forScope);
		...
		expect(self, TokenKind.Keyword, "do");
		local stat = Ast.ForInStatement(forScope, ids, expressions, nil, scope, attributes);
		stat.body = self:block(nil, stat, forScope);
		expect(self, TokenKind.Keyword, "end");
...
-- list of local variable names (returns ids, attributes for Lua 5.4)
function Parser:nameList(scope)
	local ids = {};
	local attributes = {};

	local function parseAttribute()
		...
		if(not is(self, TokenKind.Ident)) then
			self.index = self.index - 1; -- Put back the <
			return nil;
		end
		local attrIdent = get(self);
		if(attrIdent.value ~= "const" and attrIdent.value ~= "close") then
			if self.disableLog then error() end;
			logger:error(generateError(self, "expected attribute \"const\" or \"close\", got \"" .. attrIdent.value .. "\""));
		end
		expect(self, TokenKind.Symbol, ">");
		return attrIdent.value;
	end

	local attr = parseAttribute();
	local ident = expect(self, TokenKind.Ident);
...
function Parser:expressionBitwiseOr(scope)
	local lhs = self:expressionBitwiseXor(scope);

	if(self.luaVersion == LuaVersion.Lua54 and consume(self, TokenKind.Symbol, "|")) then
		local rhs = self:expressionBitwiseOr(scope);
		return Ast.BitOrExpression(lhs, rhs, true);
	end
...
function Parser:expressionBitwiseXor(scope)
	local lhs = self:expressionBitwiseAnd(scope);

	if(self.luaVersion == LuaVersion.Lua54 and consume(self, TokenKind.Symbol, "~")) then
		local rhs = self:expressionBitwiseXor(scope);
		return Ast.BitXorExpression(lhs, rhs, true);
	end
...
function Parser:expressionBitwiseAnd(scope)
	local lhs = self:expressionBitwiseShift(scope);

	if(self.luaVersion == LuaVersion.Lua54 and consume(self, TokenKind.Symbol, "&")) then
		local rhs = self:expressionBitwiseAnd(scope);
		return Ast.BitAndExpression(lhs, rhs, true);
	end

	return lhs;
end

function Parser:expressionBitwiseShift(scope)
	local lhs = self:expressionStrCat(scope);

	if(self.luaVersion == LuaVersion.Lua54 and consume(self, TokenKind.Symbol, "<<")) then
		local rhs = self:expressionBitwiseShift(scope);
		return Ast.BitwiseShiftLeftExpression(lhs, rhs, true);
	end

	if(self.luaVersion == LuaVersion.Lua54 and consume(self, TokenKind.Symbol, ">>")) then
		local rhs = self:expressionBitwiseShift(scope);
		return Ast.BitwiseShiftRightExpression(lhs, rhs, true);
	end
